...
public class EntityFactory {
    ...
    public MidnightArmour buildMidnightArmour() {
        double midnightArmorAttack = config.optInt("midnight_armour_attack");
        double midnightArmorDefence = config.optInt("midnight_armour_defence");
        return new MidnightArmour(midnightArmorAttack, midnightArmorDefence);
    }
    ...
    public Entity constructItem(JSONObject jsonEntity) {
        Position genericPos = new Position(1, 1);
        switch (jsonEntity.getString("item_type")) {
            case "Bow":
                return buildBow();
            case "MidnightArmour":
                return buildMidnightArmour();
            case "Sceptre":
                return new Sceptre(config.getInt("mind_control_duration"));
            case "Shield":
                return buildShield();
            case "InvisibilityPotion":
                int invisibilityPotionDuration = config.optInt(
                        "invisibility_potion_duration",
                        InvisibilityPotion.DEFAULT_DURATION);
                return new InvisibilityPotion(genericPos,
                        config.optInt("invisibility_potion_duration", invisibilityPotionDuration));
            case "InvincibilityPotion":
                int invincibilityPotionDuration = config.optInt("invincibility_potion_duration",
                        InvincibilityPotion.DEFAULT_DURATION);
                return new InvincibilityPotion(genericPos, invincibilityPotionDuration);
            case "Bomb":
                int bombRadius = config.optInt("bomb_radius", Bomb.DEFAULT_RADIUS);
                return new Bomb(genericPos, bombRadius);
            case "Key":
                return new Key(genericPos, jsonEntity.getInt("key"));
            case "SunStone":
                return new SunStone(genericPos);
            case "Sword":
                double swordAttack = config.optDouble("sword_attack", Sword.DEFAULT_ATTACK);
                int swordDurability = config.optInt("sword_durability", Sword.DEFAULT_DURABILITY);
                return new Sword(genericPos, swordAttack, swordDurability);
            case "Treasure":
                return new Treasure(genericPos);
            case "Wood":
                return new Wood(genericPos);
            case "Arrow":
                return new Arrow(genericPos);
            default:
                return null;
        }
    }
