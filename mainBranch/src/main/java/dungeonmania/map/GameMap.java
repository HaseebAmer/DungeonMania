...
public class GameMap {
    ...
    public void moveTo(Entity entity, Position position) {
        if (!canMoveTo(entity, position) || !entity.getMovementState().canMove()) {
        ...
        if (entity instanceof Mercenary) {
            if (((Mercenary) entity).isAllied() && Position.isAdjacent(entity.getPosition(), player.getPosition())) {
                ally = true;
            }
        }
        ...
        for (Entity e : entities) {
            if (e instanceof SwampTile && !(entity instanceof Player) && !ally) {
                entity.setMovementState(new SwampState(((SwampTile) e).getMovementFactor(), entity));
            }
        }
    ...
    public boolean removeKey() {
        if (player.countEntityOfType(Key.class) == 2) {
