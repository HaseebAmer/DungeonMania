...
public class BuildablesTest {
    ...
    @Test
    @Tag("5-2")
    @DisplayName("Test InvalidActionException is raised when the player does not have"
        + "sufficient items to build a bow or shield")
    ...
    @Test
    public void testBuildingSceptreWithKey() {
        DungeonManiaController dmc;
        dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame(
                "d_sceptreTest1", "c_sceptreTest1");
    ...
    @Test
    public void testMindControlPlayerDies() {
        DungeonManiaController dmc;
        dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame(
                "d_sceptreTest1", "c_sceptreTest1");
        ...
        res = dmc.tick(Direction.RIGHT);
        res = dmc.tick(Direction.RIGHT);
        // note: mind control duration is 4 ticks.
        buildables.add("sceptre");
        assertEquals(buildables, res.getBuildables());
        res = assertDoesNotThrow(() -> dmc.build("sceptre"));
        assertEquals(1, TestUtils.getInventory(res, "sceptre").size());

        assertEquals(1, TestUtils.countEntityOfType(TestUtils.getEntities(res), "mercenary"));
        res = assertDoesNotThrow(() -> dmc.interact(mercId)); // 1 tick
        // mercenary becomes hostile after 3 ticks
        res = dmc.tick(Direction.RIGHT);
        res = dmc.tick(Direction.LEFT);
        res = dmc.tick(Direction.RIGHT); // now player is vulnerable
        res = dmc.tick(Direction.LEFT);
        assertEquals(0, TestUtils.getInventory(res, "sceptre").size());
        assertEquals(1, TestUtils.countEntityOfType(TestUtils.getEntities(res), "mercenary"));
        assertEquals(0, TestUtils.countEntityOfType(TestUtils.getEntities(res), "player"));
    }

    @Test
    public void testSceptreWithTreasureAssassinDies() {
        DungeonManiaController dmc;
        dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame(
                "d_sceptre2", "c_spectre2");
        String assId = TestUtils.getEntitiesStream(res, "assassin").findFirst().get().getId();
        assertEquals(1, TestUtils.countEntityOfType(TestUtils.getEntities(res), "assassin"));
        ...
        res = dmc.tick(Direction.RIGHT);
        assertEquals(0, TestUtils.countEntityOfType(TestUtils.getEntities(res), "assassin"));
        assertEquals(1, TestUtils.countEntityOfType(TestUtils.getEntities(res), "player"));
